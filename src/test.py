...
TDATA = r'C:\Users\ngocq\Desktop\Project\rhinitisprediction-master\dataset\experiment2(vt_ob_bo_me)\data_test2.csv'
CHECK_DIR = '.\modelh52'
RESULT ='.\sresult(vt_ob_bm)1'
...
def write_csv(result_dir,refdataset,prediction,dataset_size):
	...
	with open(refdataset, "r") as refdataset:
		...
		with open(result, "w", newline='') as result:
			...
			for r in reader:
				r[dataset_size] = prediction[index]
				writer.writerows([r])
				index = index + 1
...
def main(args):
	...
	write_csv(result_dir,test_dataset, Y_predict,dataset_size)
